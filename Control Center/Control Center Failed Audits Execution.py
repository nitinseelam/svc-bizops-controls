...
__filename__ = "Bizops Control Center V1"
...
if __name__ == '__main__':
    try:
        ...
        query = """
            SELECT 
            ccs.Query_Name AS 'Audit_Name'
            , ccs.Audit_Number 
            , ccl.Attempt
            , ccl.[Date]
            , ccl.Duration
            , ccl.Execution_Time
            , ccl.Number_of_Results
            , ccl.Jira_Ticket
            , CAST(DATEPART(HOUR, GETDATE()) + DATEPART(MINUTE, GETDATE()) / 60.00 AS decimal(5, 2)) AS 'Current_Time'
            , ccs.Hour_Run AS 'Scheduled_Time'
            , ccs.[Priority]
                  , case
                      when ccl.attempt = 'Pass' then 'Pass'
                      when ccl.attempt = 'Fail' then 'Fail'
                      when ccl.attempt is null and (CAST(DATEPART(HOUR, GETDATE()) + DATEPART(MINUTE, GETDATE()) / 60.00 AS decimal(5, 2))) > ccs.Hour_Run then 'Past Due'
                      else 'Scheduled'
                      end as 'Status'
          FROM BizOpsAudit.dbo.ControlCenterSchedule ccs
          LEFT JOIN 
            (
                  SELECT 
                      Audit_Number, MAX(Execution_Time) AS Execution_Time
                  FROM BizOpsAudit.dbo.ControlCenterLog
                  WHERE date = cast(getdate() as date)
                  GROUP BY Audit_Number
              ) b 
              ON ccs.Audit_Number = b.Audit_Number 
          LEFT JOIN 
            bizopsAudit.dbo.ControlCenterLog ccl 
              ON ccl.Audit_Number = b.Audit_Number 
              AND ccl.Execution_Time = b.Execution_Time
          WHERE 
            ( 
              CASE 
                      WHEN DATENAME(dw,GETDATE()) = 'Monday' THEN ccs.Monday 
                      WHEN DATENAME(dw,GETDATE()) = 'Tuesday' THEN ccs.Tuesday 
                      WHEN DATENAME(dw,GETDATE()) = 'Wednesday' THEN ccs.Wednesday 
                      WHEN DATENAME(dw,GETDATE()) = 'Thursday' THEN ccs.Thursday 
                      WHEN DATENAME(dw,GETDATE()) = 'Friday' THEN ccs.Friday 
                      WHEN DATENAME(dw,GETDATE()) = 'Saturday' THEN ccs.Saturday 
                      WHEN DATENAME(dw,GETDATE()) = 'Sunday' THEN ccs.Sunday 
              END 
            ) = 1
            and ccl.Attempt = 'Fail'
        """
        conn = engine.connect()
        df = pd.read_sql(query, conn)
        conn.close()
        report = report[report['Audit_Number'].isin(list(df['Audit_Number']))] 
        print("There are {} audits that have been failed for today and will be re-run now".format(len(report)))
        ...
        if notify == 'FALSE':
            ...
            msg['Subject'] = "STG FAILURE: Control Center Failed Audits Execution"
        else:
            ...
            msg['Subject'] = "PROD FAILURE: Control Center Failed Audits Execution"
        ...
        <html>
          ...
          <body>
            <p>Hello Team,<br><br>
               Control Center Failed Audits Failed to Execute Properly. Please investigate the issue.<br><br>
               Sincerely,<br>
               Control Center - Failed Audits Execution<br>
            </p>
          </body>
          </font>
        </html>
